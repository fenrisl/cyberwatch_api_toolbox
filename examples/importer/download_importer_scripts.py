...
def download_scripts(scripts, client):
    """Get each script and put it in correct directory"""
    ...
    for script_object in scripts:
        ...
        if script.attachement:
            download_attachment(file_name, script.attachement)
    print("INFO: Script saved at {}".format(
        os.path.dirname(__file__) + '/Scripts'))


def download_attachment(path, url):
    """Download attachment if script has one"""
    attachement = requests.get(url, allow_redirects=True)
    location = os.path.join(os.path.dirname(
        __file__) + '/' + "/".join(path[:-1]))
    name = url.split("/")[-1]
    with open(location + '/' + name, 'wb') as file:
        file.write(attachement.content)
...
def launch_script():
    ...
    download_scripts(scripts, client)
