...
class TestCBWApi:
    ...
    @staticmethod
    def test_delete_cve_announcement_custom():
        """Tests for method delete_cve_announcement_custom()"""
        client = CBWApi(API_URL, API_KEY, SECRET_KEY)
        cve_code = 'CVE-2019-16768'
        with vcr.use_cassette('spec/fixtures/vcr_cassettes/delete_cve_announcement_custom.yaml'):
            response = client.delete_cve_announcement_custom(cve_code)
        assert isinstance(response, CBWCve) is True
